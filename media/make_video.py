"""Build the narrated demo video: each slide displayed for the length of its
narration, spoken by macOS TTS. Output: media/frugalrouter_demo.mp4

Usage: python media/make_video.py
"""
...
NARRATION = [
    "FrugalRouter. A token efficient routing agent for Track One of the AMD Developer Hackathon, Act Two. Built by David Castro, team Transcendiant.",
    ...
    "Every routing decision comes from measurement. A benchmark harness of two hundred twenty seven tasks, drawn from G S M eight K, Human Eval, and hand written questions, with a held out split that tuning never touched, and graders that actually execute generated code.",
    ...
    "In the dress rehearsal, on a clone of the grading machine, FrugalRouter scored ninety two point five percent on held out tasks, finished in under six minutes, and spent a small fraction of the tokens that a send everything remote baseline burns.",
    "Gemma via Fireworks is the escalation brain, not a cameo. Six of the eight categories escalate to Gemma four. Model I Ds resolve from the allowed models list at runtime, nothing is hardcoded.",
    "FrugalRouter. Built solo, in four days, fully reproducible. The harness, the tuning tools, and the ladder all ship in the public repository. Thank you.",
]
...
def main():
    ...
    for i, text in enumerate(NARRATION, 1):
        aiff = os.path.join(HERE, f"narr_{i:02d}.aiff")
        ...
        subprocess.run([
            ...
            "-loop", "1", "-i", slide, "-i", aiff,
            "-c:v", "libx264", "-tune", "stillimage", "-c:a", "aac",
            "-pix_fmt", "yuv420p", "-vf", "scale=1920:1080",
            "-af", "apad=pad_dur=0.8", "-shortest",
            seg,
        ], check=True)
        segments.append(seg)
